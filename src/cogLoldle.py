...
class CogLoldle(commands.Cog):
    ...
    @app_commands.command(
        name="splash", description="Affiche le splash du champion demander."
    )
    @app_commands.describe(
        name="Nom du champion (nom tout collé sans majuscule ex: 'kaisa')."
    )
    async def splash(self, message: discord.Interaction, name: str):
        ...
        """
        emb = discord.Embed(
            title=f"{name}", description="Work in progress", color=cons.emb_color
        )
        ...
        if self.isPlaying:
            ...
            if not champ:
                ...
                if not found:
                    ...
                    emb = discord.Embed(
                        title=f"Essai n°{len(self.loldle.guesses)}: `{name}`",
                        description=desc,
                    ...
                    emb = discord.Embed(
                        ...
                        description=self.loldle,
                        color=cons.emb_color
                    )
                    emb.set_image(url=self.loldle.champ.getUrl())
                    self.isPlaying = False
